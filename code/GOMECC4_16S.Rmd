...
```{r}
tax_new_16S = read.csv(file = "/Users/seananderson/Gomecc-4/Taxonomy16S_all.csv.gz", header=T, row.names = NULL, check.names=F,fileEncoding="UTF-8-BOM") # File compressed to reduce size
count_new_16S = read.csv(file = "/Users/seananderson/Gomecc-4/Count16S_all.csv.gz", header=T, row.names = NULL, check.names=F,fileEncoding="UTF-8-BOM") # File compressed to reduce size
...
df1_filt2 = glm.2[,c(3,5:9,12,13,16,20)]
...
More variables in Clusters 2-3 are collinear, so stick with Cluster 1 models. Spearman values for Clusters 1-3 are in Table S1. 
...
model_synecho <- glm(Abundance ~ temp  + oxygen + salinity + phosphate + nh4 + nitrate + pH_corrected + dic, data = df_synecho,family=poisson) # Poisson model
summary(model_synecho) # Summary

model_synecho_nb <- glm.nb(Abundance ~ temp  + oxygen + salinity + phosphate + nh4 + nitrate + pH_corrected + dic, data = df_synecho) # NB model
...
```{r results = FALSE}
model_synecho_nb <- glm.nb(Abundance ~ temp  + oxygen + salinity + phosphate + nh4 + nitrate + pH_corrected + dic, data = df_synecho) # Run NB model again
...
model_prochlo <- glm(Abundance ~  temp  + oxygen + salinity + phosphate + nh4 + nitrate + pH_corrected + dic, data = df_prochlo,family=poisson) # Poisson model
summary(model_prochlo) # Summary

model_prochlo_nb <- glm.nb(Abundance ~  temp  + oxygen + salinity + phosphate + nh4 + nitrate + pH_corrected + dic, data = df_prochlo) # NB model
...
```{r results = FALSE}
model_prochlo_nb <- glm.nb(Abundance ~  temp  + oxygen + salinity + phosphate + nh4 + nitrate + pH_corrected + dic, data = df_prochlo) # Run NB model again
summary(model_prochlo_nb)
stepAIC(model_prochlo_nb, direction="both", k = 3.8415) # Stepwise AIC selection

model_prochlo_sig <- glm.nb(Abundance ~  salinity + phosphate + nitrate + pH_corrected, data = df_prochlo) # Final NB model with significant factors
...
model_sar <- glm(Abundance ~  temp  + oxygen + salinity + phosphate + nh4 + nitrate + pH_corrected + dic, data = df_sar,family=poisson) # Poisson model
summary(model_sar)

model_sar_nb <- glm.nb(Abundance ~  temp  + oxygen + salinity + phosphate + nh4 + nitrate + pH_corrected + dic, data = df_sar) # NB model
...
```{r results = FALSE}
model_sar <- glm(Abundance ~temp  + oxygen + salinity + phosphate + nh4 + nitrate + pH_corrected + dic, data = df_sar,family=poisson) # Run Poisson model again
summary(model_sar)
stepAIC(model_sar, direction="both", k = 3.8415) # Stepwise AIC for factor selection

model_sar_sig <- glm(Abundance ~ temp + oxygen + nh4 + dic, data = df_sar,family=poisson) # Final model
...
#check_model(model_sar_sig) # All model performance plots
```
...
model_sar86 <- glm(Abundance ~ temp  + oxygen + salinity + phosphate + nh4 + nitrate + pH_corrected + dic, data = df_sar86,family=poisson) # Poisson model
summary(model_sar86)

model_sar86_nb <- glm.nb(Abundance ~ temp  + oxygen + salinity + phosphate + nh4 + nitrate + pH_corrected + dic, data = df_sar86) # NB model
...
```{r results = FALSE}
model_sar86 <- glm(Abundance ~ temp  + oxygen + salinity + phosphate + nh4 + nitrate + pH_corrected + dic, data = df_sar86,family=poisson) # Run Poisson model again
summary(model_sar86)
...
model_flavo <- glm(Abundance ~ temp  + oxygen + salinity + phosphate + nh4 + nitrate + pH_corrected + dic, data = df_flavo,family=poisson) # Poisson model
summary(model_flavo) # Summary

model_flavo_nb <- glm.nb(Abundance ~ temp  + oxygen + salinity + phosphate + nh4 + nitrate + pH_corrected + dic, data = df_flavo) # NB model
...
```{r results = FALSE}
model_flavo <- glm(Abundance ~temp  + oxygen + salinity + phosphate + nh4 + nitrate + pH_corrected + dic, data = df_flavo,family=poisson) # Run Poisson model again
summary(model_flavo)
...
```{r}
gomecc_all <- read.csv("/Users/seananderson/Gomecc-4/GOMECC-all-sites-final.csv", header=T, row.names = NULL, check.names=F,fileEncoding="UTF-8-BOM") # Load file of metadata from all surface sites 
...
```{r}
indic_OA <- read.csv("/Users/seananderson/Gomecc-4/OA_indic_16S.csv", header=T, row.names = NULL, check.names=F,fileEncoding="UTF-8-BOM") # Indicator ASVs file
...
indic_OA2 <- read.csv("/Users/seananderson/Gomecc-4/OA_indic2_16S.csv", header=T, row.names = NULL, check.names=F,fileEncoding="UTF-8-BOM") # Upload file
