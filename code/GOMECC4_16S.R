...
p + theme_bw() + scale_fill_manual(values = c("#BB5566","#DDAA33","#004488","#88CCEE")) +
...
# Plot diversity metrics with respect to transect - Figure S10
...
# Plot diversity metrics with respect to absolute depth - Figure S8
...
# Prepare for taxonomy tree maps - Figure S12
...
glm.2 = cluster1 %>% # Group the data based on class and preserve sample replication for the models
  ...
  summarise(temp = mean(temp),
            ...
            fluorescence = mean(fluorescence),
            Abundance = mean(Abundance),
            latitude = mean(degrees_latitude),
            longitude = mean(degrees_longitude),
            .groups = 'drop')
...
df1_filt2 = glm.2[,c(3,5:8,9,10,13:14,17,21)] # Re-do to add back in the region column needed for outlier removal
...
train_index <- createDataPartition(df_sar_filt$Strata, p = 0.8, list = FALSE) # Define train set as 80%
...
train_index <- createDataPartition(df_pro_filt$Strata, p = 0.8, list = FALSE) # Define train set as 80%
...
ggplot(combined_data, aes(x = LogAbundance, y = Predicted)) +
  ...
  annotate("text", x = Inf, y = -Inf, hjust = 1.1, vjust = -1.5, label = paste0("RMSE (Train) = ", round(train_rmse, 2), "\nRMSE (Test) = ", round(test_rmse, 2)), size = 4.5) + 
  labs(title = "Observed vs. Predicted Log Abundance (GAM)", subtitle = equation_text, x = "Observed log(Abundance + 1)", y = "Predicted log(Abundance + 1)", fill = "Data Set") +
  xlim(0, 7.5) + ylim(0, 7.5) + theme_bw()
...
train_index <- createDataPartition(df_syne_filt$Strata, p = 0.8, list = FALSE) # Define train set as 80%
...
train_index <- createDataPartition(df_sar86_filt$Strata, p = 0.8, list = FALSE) # Define train set as 80%
...
train_index <- createDataPartition(df_flavo_filt$Strata, p = 0.8, list = FALSE) # Define train set as 80%
...
gomecc_all_sar = gomecc_all_trans[,c(13,15,17,18,20:21)] # Subset to variables in final SAR11 model
...
odv_sar = cbind(new_val_sar, gomecc_all_trans[4:6])

gomecc_all_sar86 = gomecc_all_trans[,c(9,13,15,17,19)] # Subset to variables in final SAR86 model
...
gomecc_all_flavo = gomecc_all_trans[,c(9,13,17:18,20:21)] # Subset to variables in final Flavobacteriales model
...
gomecc_all_pro = gomecc_all_trans[,c(9,15,17:19)] # Subset to variables in final Prochlorococcus model
...
gomecc_all_syne = gomecc_all_trans[,c(13,17:19,21)] # Subset to variables in final Synechococcus model
...
#write.csv(gomecc_all_trans, "ODV_factors_16S.csv", row.names = T) # Write .csv file of log factors
...
## GOM map with points filled based on 16S cluster assignments at each depth - Figure S5
