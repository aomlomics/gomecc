...
ps_clr <- microbiome::transform(ps_rare, "clr") # Centered log transform rarefied data
...
# Plot environmental factors across the three clusters - Figure S8
...
# Plot diversity metrics with respect to transect - Figure S13
...
# Plot diversity metrics with respect to absolute depth - Figure S11
...
# Prepare for taxonomy tree maps - Figure S15
...
# Perform centered log-ratio (CLR) transformation of the ASV count data and save as separate data frame; will merge to compare with log abundance downstream
otu <- as.data.frame(t(otu_table(ps_subset))) # Select ASV count table
...
x1 <- dplyr::left_join(x1, otu_clr_long, by = c("Sample", "OTU")) # Merge with melted phyloseq table; now CLR abundance values have their own column
...
train_index <- suppressWarnings(createDataPartition(df_syn_filt$Strata, p = 0.8, list = FALSE)) # Define train set as 80%; avoid warning message, some samples no longer there after the outlier removal
...
train_index <- suppressWarnings(createDataPartition(df_dino_filt$Strata, p = 0.8, list = FALSE)) # Define train set as 80%
...
ggplot(combined_data, aes(x = LogAbundance, y = Predicted)) +
  ...
  annotate("text", x = Inf, y = -Inf, hjust = 1.1, vjust = -1.5, label = paste0("RMSE (Train) = ", round(train_rmse, 2), "\nRMSE (Test) = ", round(test_rmse, 2)), size = 4.5) + 
  labs(title = "Observed vs. Predicted Log Abundance (GAM)", subtitle = equation_text, x = "Observed log(Abundance + 1)", y = "Predicted log(Abundance + 1)", fill = "Data Set") +
  xlim(5.8, 7.7) + ylim(5.8, 7.7) + theme_bw()
...
train_index <- suppressWarnings(createDataPartition(df_prym_filt$Strata, p = 0.8, list = FALSE)) # Define train set as 80%
...
train_index <- suppressWarnings(createDataPartition(df_sag_filt$Strata, p = 0.8, list = FALSE)) # Define train set as 80%
...
## Compare Gaussian GAMs with CLR vs. log transformed read counts for all 18S groups
# Function to build the plot of predicted vs. observed abundances - Figure S5
...
# Specify groups
group_list <- list(Syn = df_syn_filt, Dino = df_dino_filt, Prym = df_prym_filt, Sag = df_sag_filt)
...
# Final figure panel for all 4 groups
final_panel <- wrap_plots(plotlist = plot_list, ncol = 2)
final_panel
#ggsave(filename = "GAM_log_clr_18S.pdf", plot = last_plot(), device = "pdf", path = NULL, scale = 1, width = 10, height = 10, dpi = 150)
...
gomecc_all_pred <- gomecc_all # Rename for predictions
...
gomecc_all_syn = gomecc_all_pred[,c(9,15,18:21)] # Subset to variables in final Syndiniales model
...
odv_syn = cbind(new_val_syn, gomecc_all_pred[4:6])

gomecc_all_dino = gomecc_all_pred[,c(9,13,15,17:18,20:21)] # Subset to variables in final Dinophyceae model
...
gomecc_all_prym = gomecc_all_pred[,c(9,13,15,17:19)] # Subset to variables in final Prymnesiophyceae model
...
gomecc_all_sag = gomecc_all_pred[,c(17,20:21)] # Subset to variables in final Sagenista model
...
#write.csv(gomecc_all_pred, "ODV_factors.csv",row.names = T) # Write .csv file of log factors
